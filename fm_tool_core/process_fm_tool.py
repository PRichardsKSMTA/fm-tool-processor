...
"""
# Allow postponed evaluation of type hints
from __future__ import annotations

import argparse  # Parse command line arguments
import json  # Work with JSON data
import logging  # Log progress and errors
import os  # Access environment variables
import sys  # Access stdin, stdout, and system functions
import time  # Sleep and measure elapsed time
...
from typing import Any, Dict, List  # Type annotations for data structures
from urllib.parse import urlparse  # Split URL strings

import psutil  # System monitoring utilities

# ────────────────────────────── pyodbc (optional) ──────────────────────────
# pyodbc is only needed for SQL operations; if missing we continue without SQL
try:
...
# Import configuration constants and helper functions from this package
...
def _update_status(scac: str, state: str, log: logging.Logger) -> None:
    # Call the stored procedure that updates processing status
    _exec_proc(
...
def _reset_status(scac: str, state: str, log: logging.Logger) -> None:
    # Call the stored procedure that resets processing status
    _exec_proc(
...
def _fifo_sort(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # Find a key that looks like a timestamp
    key = next(
        (
            k
...
def run_flow(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Extract input rows and options
    ...
    # Determine the SCAC code from the first row
    op_code = rows[0]["SCAC_OPP"]
    scac = op_code.split("_", 1)[0].upper()

    # Mark the start of processing in the database
...
def _cli() -> None:
    # Command line helper for manual execution
